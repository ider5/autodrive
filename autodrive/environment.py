...
class Environment:
    ...
    def _draw_lane_labels(self, ax):
        ...
        for i in range(self.num_lanes):
            ...
            lane_text = f"车道 {lane_id}" if i18n.labels is None else f"Lane {lane_id}"
    ...
    def plot_environment(self, ax=None):
        ...
        # 添加起点和终点标签
        start_label = '起点' if i18n.labels is None else i18n.labels.get('起点', 'Start')
        end_label = '终点' if i18n.labels is None else i18n.labels.get('终点', 'End')
        ...
        title = '道路环境' if i18n.labels is None else i18n.labels.get('道路环境', 'Road Environment')
        ax.set_title(title)
